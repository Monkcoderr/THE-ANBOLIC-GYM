...
/**
 * Next expiry on renewal: always the member's billing day, far enough ahead
 * that the period they just paid for is honoured in full.
 *
 *   02-May → 02-Jun → 02-Jul → 02-Aug      (30-day plans, paid any day)
 *   23-Aug → 23-Sep → 23-Oct → 23-Nov
 *
 * Coverage runs from the LATER of the current expiry and today:
 *
 *  • Paid early or on time — the new period extends the existing one, so the
 *    payment date is irrelevant and the billing day never moves.
 *  • Paid after lapsing — the period starts now rather than being back-dated
 *    into the gap, so a member who disappeared for five months and buys one
 *    month gets a month, not the two days left until their next billing date.
 *
 * The paid-for date (`base + N months`) is then moved to the NEAREST billing
 * date that still adds time. Nearest rather than the next one along, because a
 * record whose stored expiry drifted off the billing day (legacy data, or a
 * hand-edited date) must be pulled back into line without either granting a
 * free month or charging a month for a few days — the correction is at most
 * half a month in either direction, and ties go to the member.
 */
export function computeRenewalExpiry(
  ...
  const current = utcDateOnly(currentExpiry);
  ...
  // The billing dates on either side of the target. Offsets past +1 only come
  // into play for a badly corrupted record whose nearer billing dates all fall
  // on or before the base — a renewal must always add time.
  for (let offset = -1; offset <= 3; offset += 1) {
    const candidate = applyAnchorDay(addUtcMonths(target, offset), anchorDay);
